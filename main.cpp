...
void imgOps(unsigned char operation, unsigned char openFlag)
{
    ...
    case IMG_OPEN_FILE:
        ...
        if (!(file = fopen(filename, "r"))) {
            MessageBox(NULL, L"Arquivo nao encontrado!", L"ERRO", MB_OK);
            return;
        }
        fclose(file);
...
int saveFile(HWND hWnd)
{
    ...
    sfn.lpstrFilter = L"JPEG Image\0*.JPG\0PNG Image\0*.PNG\0BMP Image\0*.BMP\0";
    sfn.nFilterIndex = 1;
    sfn.lpstrDefExt = L"JPG\0PNG\0BMP\0";
